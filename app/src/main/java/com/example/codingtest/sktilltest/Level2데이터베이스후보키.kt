...
*  "이름"은 후보 키가 될 수 없다. 그러나, 만약 ["이름", "전공"]을 함께 사용한다면 릴레이션의 모든 튜플을 유일하게 식별 가능하므로 후보 키가 될 수 있게 된다.
...
* */
class Level2데이터베이스후보키 {
    fun solution(relation: Array<Array<String>>): Int {
        var answer = 0
        //var rrrr = mutableListOf<MutableList<String>>(mutableListOf<String>())//= arrayOf<Array<String>>(arrayOf<String>())
        for (j in relation[0].indices) {
            var count = 0
            var rrrr = mutableListOf<String>()
            for (i in relation.indices) {
                if (rrrr.isNotEmpty()) {
                    if (!rrrr.contains(relation[i][j])) {
                        rrrr.add(relation[i][j])
                    } else {
                        count++
                    }
                }
