...
class Level2메뉴구성 {
    fun solution(orders: Array<String>, course: IntArray): Array<String> {
        ...
        var orderarr = mutableListOf<String>()
        for(oder in orders){
            var or = oder.split("")
            for(o in or){
                if(!orderarr.contains(o) && o.isNotEmpty()){
                    orderarr.add(o)
                    println(o)
                }
            }
        ...
        var listo = orderarr.sorted()

        for(i in orderarr.indices){
            val a = mutableListOf<List<String>>()
            combination(a, listo,Array<Boolean>(listo.size){false},0,i)
            listo.forEach { println(it) }
        }
        return answer
    }

    fun combination(answer: MutableList<List<String>>, el: List<String>, ck: Array<Boolean>, start: Int, target: Int){
        if(target == 0) {
